                ...
                {appointments.map((item, index) => (
                    <div key={index} className='grid grid-cols-[1fr_2fr] gap-4 sm:flex sm:gap-6 py-4 border-b'>
                        ...
                        <div className='flex-1 text-sm text-[#5E5E5E]'>
                            ...
                            {!item.cancelled && !item.payment && <p className='mt-3 rounded border border-blue-100 bg-blue-50 p-3 text-sm text-blue-800'>Contact details unlock after payment.</p>}
                            {!item.cancelled && item.payment && <div className='mt-3 rounded border border-green-200 bg-green-50 p-3 text-sm text-green-800'><p className='font-semibold'>Contact Doctor</p><p>{item.docData.phoneNumber || 'Phone number unavailable'}</p><p>{[item.docData.clinicAddress?.line1, item.docData.clinicAddress?.line2, item.docData.clinicAddress?.village, item.docData.clinicAddress?.taluka, item.docData.clinicAddress?.city, item.docData.clinicAddress?.state, item.docData.clinicAddress?.zipcode].filter(Boolean).join(', ') || 'Clinic address unavailable'}</p><div className='mt-2 flex flex-wrap gap-2'><a href={`tel:${item.docData.phoneNumber}`} className='rounded bg-green-700 px-3 py-1 text-white'>Call Now</a>{item.docData.phoneNumber && <a href={`https://wa.me/${item.docData.phoneNumber.replace(/\D/g, '')}`} target='_blank' rel='noreferrer' className='rounded border border-green-700 px-3 py-1 text-green-700'>WhatsApp</a>}<a href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent([item.docData.clinicAddress?.line1, item.docData.clinicAddress?.line2, item.docData.clinicAddress?.village, item.docData.clinicAddress?.taluka, item.docData.clinicAddress?.city, item.docData.clinicAddress?.state, item.docData.clinicAddress?.zipcode].filter(Boolean).join(', '))}`} target='_blank' rel='noreferrer' className='rounded border border-green-700 px-3 py-1 text-green-700'>Directions</a></div></div>}
